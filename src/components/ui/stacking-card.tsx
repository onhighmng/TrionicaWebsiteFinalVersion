  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`flex relative -top-[25%] ${customHeight || 'h-[520px]'} w-[90%] max-w-6xl rounded-[32px] overflow-hidden origin-top`}
      >
        ...
        {url && (
          <div
            className={`relative w-[40%] h-full overflow-hidden flex-shrink-0 z-10`}
          >
            <motion.div
        ...
        <div className={`flex-1 px-20 py-16 flex flex-col justify-center relative z-10 ${url ? '' : 'items-start'}`}>
          {/* Decorative elements */}
          ...
          <div className="space-y-6 max-w-3xl">
            <h2 className='text-[66px] font-bold text-white tracking-tight leading-tight drop-shadow-sm'>
              {title}
            </h2>
            
            <div className='text-[22px] leading-[1.9] text-white/95 drop-shadow-sm space-y-4'>
              {description.split('\n').map((line, idx) => (
                line.trim() && (
                  <p key={idx} className="flex items-start gap-3">
                    {line.startsWith('•') ? (
                      <>
                        <span className="text-white/70 text-2xl leading-none mt-0.5">•</span>
          ...
          <div className="absolute bottom-12 right-20 w-24 h-24 border-r-2 border-b-2 border-white/20 rounded-br-3xl"></div>
        </div>
      </motion.div>
    </div>
  );
